...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    -h|--help)
      ...
      echo "  --title        Blog post title (required, used as identifier)"
...
# URL-encode the title for the endpoint
...
jq -n \
  ...
  --arg content "$CONTENT" \
  '{blogPost:{description:$description,tags:$tags,content:$content}}' \
| curl -X PUT "$API_URL/$ENCODED_TITLE" \
